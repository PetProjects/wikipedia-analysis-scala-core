...
trait ExtractionLogger extends BasicStackedDatesExtractor {
  ...
  def tryExtract(dateStartInd: Int, m: Regex.Match)(pDate: => WikiDate): DateExtraction =
    try {
      DateExtraction(pDate, dateStartInd, m)
    }
    catch {
      case ex: Exception =>
        logExtractionIssue(s"Failed extracting date for '${m.matched}'\n$ex")
        DateExtraction(WikiDate.NoDate, dateStartInd, m)
    }
}
